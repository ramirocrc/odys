...
def _cvar_expr(model: EnergyMILPModel, cvar: CVaRTerm) -> linopy.LinearExpression:
    probs = model.parameters.scenarios.scenario_probabilities
    expected_shortfall = (probs * model.vars.shortfall_revenue).sum(ModelDimension.Scenarios)
    return model.vars.value_at_risk - (1 / (1 - cvar.confidence_level)) * expected_shortfall
