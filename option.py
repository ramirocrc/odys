...
class EnergyModel:
    ...
    def add_variable(self, name: str, variable: pyo.Var) -> None:
        setattr(self.model, name, variable)

    def add_constraint(self, name: str, constraint: pyo.Constraint) -> None:
        setattr(self.model, name, constraint)

    def add_objective(self, name: str, objective: pyo.Objective) -> None:
        setattr(self.model, name, objective)

    @property
    def time(self) -> pyo.Set:
        return self.model.time

    @property
    def generators(self) -> pyo.Set:
        return self.model.generators

    @property
    def batteries(self) -> pyo.Set:
        return self.model.batteries
...
class MathModelBuilder:
    ...
    def build_model(self) -> EnergyModel:
        ...
        model.model.time = pyo.Set(initialize=time_indices)
        model.model.generators = pyo.Set(initialize=generator_indices)
        model.model.batteries = pyo.Set(initialize=battery_indices)

        # === Variables ===
        model.add_variable(
            "generator_power",
            pyo.Var(model.time, model.generators, domain=pyo.NonNegativeReals),
        )
        model.add_variable(
            "battery_charge",
            pyo.Var(model.time, model.batteries, domain=pyo.NonNegativeReals),
        )
        model.add_variable(
            "battery_discharge",
            pyo.Var(model.time, model.batteries, domain=pyo.NonNegativeReals),
        ...
        model.add_variable(
            "battery_charge_mode",
            pyo.Var(model.time, model.batteries, within=pyo.Binary),
        )

        # === Constraints ===
        model.add_constraint(
            "power_balance",
            pyo.Constraint(model.time, rule=self.power_balance_rule),
        )
        model.add_constraint(
            "generator_limit",
            pyo.Constraint(model.time, model.generators, rule=self.generator_limit_rule),
        )
        model.add_constraint(
            "battery_charge_limit",
            pyo.Constraint(model.time, model.batteries, rule=self.battery_charge_limit_rule),
        )
        model.add_constraint(
            "battery_discharge_limit",
            pyo.Constraint(model.time, model.batteries, rule=self.battery_discharge_limit_rule),
        )
        model.add_constraint(
            "battery_soc_dynamics",
            pyo.Constraint(model.time, model.batteries, rule=self.battery_soc_dynamics_rule),
        )
        model.add_constraint(
            "battery_soc_bounds",
            pyo.Constraint(model.time, model.batteries, rule=self.battery_soc_bounds_rule),
        )
        model.add_constraint(
            "battery_soc_terminal",
            pyo.Constraint(model.batteries, rule=self.battery_soc_terminal_rule),
        )
